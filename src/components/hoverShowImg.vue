...
const followOrNot = (index) => {
    ...
    emits('followUser', !props.userInfo.followPerson, props.index)
    // 发送请求 来 给后台响应数据
}   
const goToHome = () => {
...
<template>
 <el-popover
    ...
    >
        ...
        <div v-if="props.userInfo.self === false"  @click="followOrNot(index)" style="cursor: pointer; width: 260px; height: 35px; border-radius: 5px; background-color: gainsboro; display: flex; align-items: center;justify-content: center; transition: 0.3s;" :style="props.userInfo.followPerson === false ? 'background-color:#3FBB6B;':'background-color:gainsboro'">
            <span :style="props.userInfo.followPerson === true ? 'color: gray; font-size: larger': 'color: white; font-size: larger'">{{ props.userInfo.followPerson === false ? "+ 关注": "取消关注" }}</span> 
        </div>
